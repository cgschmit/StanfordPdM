...
for i in range(np.shape(all_feats_new)[0]):
    ...
    pt1_new=tuple2array(pt1_new)
    pt2_new=tuple2array(pt2_new)
    ...
    # Calculate Position of Features on the big map
    center_current_alt=applyRotationTo(all_feats_current[i], frame_x_ROI, frame_y_ROI, img_2D_rot_angle[i])
    center_new_alt=applyRotationTo(all_feats_new[i], frame_x_ROI, frame_y_ROI, img_2D_rot_angle[i+1])
